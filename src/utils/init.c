...
t_ray   ray_init(t_vec3 origin, t_vec3 direction)
{
    ...
    r.origin = origin;
    r.direction = vec3_normalize(direction); // Crucial: la dirección del rayo debe estar normalizada
    return (r);
}
...
t_camera camera_init(t_vec3 position, t_vec3 orientation, double fov)
{
    ...
    c.position = position;
    c.orientation = vec3_normalize(orientation); // La orientación de la cámara debe estar normalizada
    c.fov = fov;
    return (c);
}
...
t_cylinder cylinder_init(t_vec3 position, t_vec3 axis, double radius, double height)
{
    ...
    cy.axis = vec3_normalize(axis); // Crucial: el eje del cilindro debe estar normalizado
    cy.radius = radius;
    cy.height = height;
    return (cy);
}
