...
2.  **Estructuración de Datos:** La información parseada se organiza y almacena meticulosamente dentro de una estructura de datos coherente en memoria que representa toda la escena.[4, 7]
3.  **Configuración del Entorno Gráfico:** De forma concurrente o subsiguiente, se inicializa la biblioteca gráfica MiniLibX y se crea la ventana de visualización.[8, 9]
4.  **Generación y Trazado de Rayos:** El núcleo del proceso de renderizado implica iterar a través de cada píxel de la imagen de salida. Para cada píxel, se genera un rayo único, que se origina desde la cámara virtual y se extiende hacia la escena 3D.[4, 6, 8]
5.  **Intersección y Sombreado:** Este rayo se "traza" para determinar si intersecta con algún objeto en la escena. Si ocurre una intersección, el color en ese punto se calcula basándose en modelos de iluminación complejos y cálculos de sombras.[3, 4, 6, 10]
6.  **Dibujo de Píxeles:** El color calculado se escribe en el búfer de imagen de MiniLibX, "pintando" efectivamente el píxel.[11, 12]
7.  **Visualización e Interacción:** Finalmente, la imagen completamente renderizada se muestra en la ventana de MiniLibX, y el programa entra en un bucle de eventos para manejar las entradas del usuario.[13, 14]

La descripción de la escena en sí es un archivo de texto estructurado que contiene definiciones para la luz ambiental, las propiedades de la cámara, varias fuentes de luz y primitivas geométricas como esferas, planos y cilindros.[4, 7, 15]
...
Un plano se define por un punto `S` en el plano y un vector normal `N` perpendicular a él.[30] La ecuación implícita de un plano es:
`N ⋅ (P - S) = 0` [27, 30, 31]
...
La normal de la superficie para un plano es simplemente su vector normal `N` predefinido.[21]
...
*   **Luz Ambiental (Ambient):** Representa la luz indirecta que se dispersa por toda la escena, proporcionando una iluminación de base uniforme.[7, 22, 23] Se calcula multiplicando el color del objeto por la relación y el color de la luz ambiental global.[22]
    `Color_Ambient = Objeto.Color * Ambient.Ratio * Ambient.Color` [22, 34, 35]
...
`Color_Total = Color_Ambient + ∑ (Color_Difuso + Color_Especular)` (sumado para cada fuente de luz).[22, 34, 35]

#### Sombras (Shadows)

Para simular sombras, se lanza un "rayo de sombra" desde el punto de intersección `P` en la superficie hacia cada fuente de luz `Li`.[3, 4, 10, 35] Es crucial desplazar ligeramente el origen del rayo de sombra fuera de la superficie para evitar que intersecte el propio objeto del que proviene.[34]
`Rayo_Sombra_Origen = P + ε * L_dir` (donde `ε` es un valor pequeño, como 0.001) [34]
`Rayo_Sombra_Direccion = normalize(Li.position - P)`

Si este rayo de sombra intersecta cualquier otro objeto en la escena *antes* de llegar a la fuente de luz, significa que la fuente de luz está ocluida y el punto `P` está en sombra con respecto a esa luz.[3, 4, 10, 35] En este caso, la contribución difusa y especular de esa luz en particular para el punto `P` se anula (o se reduce significativamente).[35]

#### Reflexiones (Reflections) y Refracciones (Refractions) (Bonus)

Para un mayor realismo, `miniRT` puede extenderse para incluir reflexiones y refracciones. Esto implica un trazado de rayos recursivo.[35]
...
El primer paso es inicializar la conexión con el entorno gráfico utilizando `mlx_init()`.[4, 8, 9] Esta función devuelve un puntero `void *` que representa la conexión con MiniLibX; este puntero es crucial y debe almacenarse, ya que se requerirá para la mayoría de las funciones posteriores de MiniLibX.[9]

Una vez inicializado MiniLibX, se puede crear una ventana utilizando `mlx_new_window(mlx_ptr, width, height, title)`.[8, 9] Esta función devuelve un puntero `void *` al identificador de la ventana, que también debe almacenarse. La ventana se abrirá con las dimensiones especificadas y un título.[9]
...
1.  **Función `main`:**
    ...
    *   Inicializar MiniLibX (`mlx_init()`) y crear la ventana (`mlx_new_window()`).[8, 9]
    *   Inicializar la estructura de datos `t_scene`.
    *   Invocar el parser para leer el archivo `.rt` y poblar la estructura `t_scene` con todas las luces, la cámara y los objetos.[4, 7]
...
2.  **Bucle de Renderizado:**
    ...
    *   Para cada píxel:
        *   Construir un rayo primario desde la cámara a través del píxel actual.[4, 6, 8]
        ...
        *   Escribir el color calculado en el búfer de imagen de MiniLibX en la posición del píxel actual (`ft_put_pixel` o similar).[11, 12]
...
La Norminette es un estándar de codificación estricto impuesto por la Escuela 42 para todos los proyectos en C del Common Core.[16, 17] Su propósito es fomentar la claridad, la legibilidad y la modularidad del código, aunque a menudo se percibe inicialmente como una restricción.

### Reglas Clave de la Norminette

*   **Nombres de Archivos y Directorios:** Solo minúsculas, dígitos y guiones bajos (snake_case).[16, 17]
*   **Nombres de Identificadores:**
    *   Variables y funciones: Solo minúsculas, dígitos y guiones bajos (snake_case).[16, 17]
    *   Nombres de estructuras: Deben comenzar con `s_` (por ejemplo, `s_sphere`).[16, 17]
    *   Nombres de `typedef`: Deben comenzar con `t_` (por ejemplo, `t_sphere`).[16, 17]
    *   Nombres de uniones: Deben comenzar con `u_`.[16, 17]
    *   Nombres de enumeraciones: Deben comenzar con `e_`.[16, 17]
    *   Nombres de globales: Deben comenzar con `g_`.[16, 17]
    *   Todos los identificadores deben estar en inglés y ser lo más explícitos o mnemotécnicos posible.[16, 17]
*   **Longitud de Función:** Cada función debe tener un máximo de 25 líneas, sin contar las llaves de la función.[16, 17] Esto promueve la descomposición de tareas en funciones pequeñas y de propósito único.
*   **Longitud de Línea:** Cada línea debe tener un máximo de 80 columnas de ancho, incluyendo los comentarios.[16, 17] Las tabulaciones cuentan como el número de espacios que representan, no como una sola columna.[16, 17]
*   **Espacios y Saltos de Línea:**
    *   Una instrucción por línea.[16, 17]
    *   Cada función debe estar separada por una línea vacía.[16, 17]
    *   Las líneas vacías deben estar realmente vacías (sin espacios ni tabulaciones).[16, 17]
    *   Una línea nunca puede terminar con espacios o tabulaciones.[16, 17]
    *   No puede haber dos espacios consecutivos.[16, 17]
    *   Debe haber un salto de línea después de cada llave de apertura o final de estructura de control.[16, 17]
    *   Cada coma o punto y coma debe ir seguido de un espacio, a menos que sea el final de una línea.[16, 17]
    *   Cada operador u operando debe estar separado por un único espacio.[16, 17]
    *   Cada palabra clave de C debe ir seguida de un espacio, excepto para las palabras clave de tipos (como `int`, `char`, `float`, etc.) y `sizeof`.[16, 17]
*   **Declaraciones de Variables:**
    *   Deben estar al principio de una función.[16, 17]
    *   Una sola declaración de variable por línea.[16, 17]
    *   Las declaraciones y las inicializaciones no pueden estar en la misma línea, excepto para variables globales (cuando se permite), estáticas y constantes.[16, 17]
    *   Debe haber una línea vacía entre las declaraciones de variables y el resto de la función.[16, 17]
    *   No se permiten otras líneas vacías dentro de una función.[16, 17]
    *   Las variables no pueden ser más de 5 por función.[16]
    *   Los asteriscos de los punteros deben ir pegados a los nombres de las variables (por ejemplo, `char *my_string;`).[16, 17]
*   **Estructuras de Control:** Las estructuras de control (`if`, `while`, etc.) deben usar llaves, a menos que contengan una sola instrucción en una sola línea.[16, 17]
...
El multithreading permite dividir un proceso en múltiples unidades más pequeñas llamadas hilos (threads), que pueden ejecutarse simultáneamente.[47] En el contexto del ray tracing, esto significa que diferentes partes de la imagen pueden ser renderizadas por diferentes hilos de forma concurrente.[48, 49] Dado que el cálculo del color de un píxel es en gran medida independiente del cálculo de otros píxeles (ignorando efectos como la iluminación global que podrían introducir dependencias), el ray tracing se considera un "problema vergonzosamente paralelo".[48]

La paralelización puede mejorar significativamente el rendimiento del renderizado, especialmente en sistemas con múltiples núcleos de CPU.[47, 50]

### Implementación con Pthreads en C

En C, la biblioteca POSIX Threads (pthreads) se utiliza para implementar multithreading. Para usarla, se debe incluir el archivo de cabecera `<pthread.h>` y compilar el programa con la bandera `-lpthread`.[47]

El proceso básico de multithreading con pthreads implica:
1.  **Creación de Hilos:** Utilizar `pthread_create()` para crear un nuevo hilo.[47] Esta función toma:
    ...
    *   Un puntero a la función que el hilo ejecutará (la "rutina del hilo"). Esta función debe tener la firma `void* (*start_routine)(void*)`.[47]
    *   Un único argumento que se pasará a la función del hilo (a menudo `NULL` o un puntero a una estructura de datos que contenga la información necesaria para el hilo).[47]

2.  **Unión de Hilos:** Utilizar `pthread_join()` para esperar a que un hilo termine su ejecución.[47] Esto es crucial para asegurar que todos los cálculos de renderizado hayan finalizado antes de que el programa principal intente mostrar la imagen o salir.

### Estrategias de Paralelización

Existen varias formas de dividir el trabajo de renderizado entre los hilos:
*   **División por Filas/Columnas:** Asignar un conjunto de filas o columnas de píxeles a cada hilo.[49] Por ejemplo, un hilo podría renderizar las filas pares y otro las impares, o dividir la imagen en franjas horizontales o verticales.[48]
*   **División por Tiles (Baldosas):** Dividir la imagen en baldosas (por ejemplo, de 64x64 o 32x32 píxeles) y asignar dinámicamente estas baldosas a los hilos disponibles.[49] Esta estrategia es a menudo más eficiente en términos de equilibrio de carga, ya que los hilos pueden tomar nuevas baldosas a medida que terminan las suyas, minimizando el tiempo de inactividad si algunas regiones son más complejas de renderizar que otras.[49]
...
*   **Sobrecarga de Hilos:** La creación y gestión de hilos conlleva una sobrecarga. Si el trabajo por hilo es demasiado pequeño, la sobrecarga puede superar las ganancias de rendimiento.[51]
*   **Balanceo de Carga:** Es vital distribuir el trabajo de manera uniforme entre los hilos para evitar que algunos hilos terminen mucho antes que otros, dejando núcleos de CPU inactivos.[48] La división por baldosas con asignación dinámica es una buena estrategia para esto.[49]
*   **Condiciones de Carrera (Race Conditions):** Si múltiples hilos acceden y modifican datos compartidos sin la sincronización adecuada, pueden ocurrir condiciones de carrera, llevando a resultados incorrectos o comportamiento impredecible.[47, 52]
    *   **Mutexes:** Para proteger los datos compartidos (como un contador de píxeles procesados o una estructura de datos de escena que podría ser modificada por múltiples hilos, aunque en ray tracing puro esto es menos común ya que los píxeles son independientes), se pueden usar mutexes (locks).[52, 53] Un mutex asegura que solo un hilo pueda acceder a una sección crítica del código a la vez.[53]
    *   `pthread_mutex_init()`: Inicializa un mutex.[53]
    *   `pthread_mutex_lock()`: Bloquea el mutex; si ya está bloqueado, el hilo espera.[52, 53]
    *   `pthread_mutex_unlock()`: Libera el mutex.[52, 53]
    *   `pthread_mutex_destroy()`: Elimina un mutex.[53]
*   **Falsos Compartidos (False Sharing):** Un problema de rendimiento en sistemas con caché que puede ocurrir si diferentes hilos modifican datos que residen en la misma línea de caché, incluso si los datos lógicamente no están relacionados. Esto puede llevar a invalidaciones de caché excesivas y reducir el rendimiento. La asignación de bloques contiguos de píxeles (como en la división por baldosas) tiende a mitigar esto en comparación con la asignación de píxeles intercalados.[48]
